...
#ifndef STROBE_MODES_CLASSIC_C
#define STROBE_MODES_CLASSIC_C

#include "strobe-modes-classic.h"

#ifdef USE_STROBE_STATE
uint8_t strobe_state_classic(Event event, uint16_t arg) {
    static int8_t ramp_direction = 1;

    // 'st' reduces ROM size slightly
    strobe_mode_classic_te st = strobe_type;
    ...
    // 1 click: off
        ...
        // 2 clicks: switch to strobe_a_mode
    else if (event == EV_2clicks) {
        set_state(strobe_state_a, 0);
        set_level(0);
        return MISCHIEF_MANAGED;
    }
    // 3 clicks: rotate backward through strobe/flasher modes
    else if (event == EV_3clicks) {
        set_state(strobe_state_b, 0);
        set_level(0);
        return MISCHIEF_MANAGED;
    }
    // 4 clicks: rotate through strobe/flasher modes
    else if (event == EV_4clicks) {
        // 0 - candle
        // 1 to 4 - ascending strobe
        // 5 to 8 - thin thin thick
        strobe_type = (st + 1) % NUM_STROBES_CLASSIC;
...
inline void strobe_state_classic_iter() {
